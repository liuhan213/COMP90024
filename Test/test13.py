...
def get_coords_list(ins_json_address):
    with open(ins_json_address) as f:
        ...
        for line in f:
            try:
                coord_one = {}
                line_each = json.loads(line[0:len(line)-2])
...
if size > 1:
    if rank==0:
        ...
        print('size>1: start_time:',start_time)

        # get coord list
        coord_list = get_coords_list('mediumInstagram.json')
    ...
    if rank ==0:
        ...
        # finish timing
        print('size>1:endtime:',time.time())
        ...
        start_time = time.time()
        print('size1: start_time:',start_time)
